...
public class CursorSetting : MonoBehaviour
{
    ...
    private void OnGUI()
    {
        ...
        if ((int)GameManager.nowCH1State % 2 != 0 && CursorNotTrigger)//在背面
        {
            Vector2 tempWorldPos = Camera.main.ScreenToWorldPoint(mousePos);
            if (tempWorldPos.x < 7.8 && tempWorldPos.x > -8.1 && tempWorldPos.y < -2.7 && tempWorldPos.y > -5 && !userManager.isZoomin && !visualManager.ShowBag)
            {
        ...
        {
            if ((int)GameManager.nowCH1State % 2 == 0)//正面
            {
    ...
    {
        if ((int)GameManager.nowCH1State % 2 != 0 && visualManager.ShowBag)
        {
